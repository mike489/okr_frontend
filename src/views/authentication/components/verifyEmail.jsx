...
const EmailVerification = ({ email, onClose }) => {
    ...
    fetch(Api, {
      ...
      .then((response) => {
        if (response.success) {
          ...
          navigate('/reset-password', { state: { token: response.data?.access_token } });
        } else {
  ...
  return (
    <DrogaCard sx={{ position: 'relative', p: 4 }}>
      <IconButton onClick={onClose} sx={{ position: 'absolute', top: 3, right: 3, color: theme.palette.primary.main }}>
        <IconX size={22} />
      </IconButton>

      {verified ? (
        <AnimatedSuccess initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} sx={{ textAlign: 'center' }}>
          <AnimateIcon
            initial={{ y: -100 }}
            animate={{ y: 0 }}
            transition={{
        ...
        <>
          ...
          <Button
            ...
            onClick={() => VerifyCode()}
          >
            {isSubmitting ? <ActivityIndicator size={22} sx={{ color: theme.palette.background.default }} /> : 'Verify'}
          </Button>
        </>
      )}
