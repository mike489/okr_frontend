    ...
    fetch(Api, {
      ...
      .then((response) => {
        if (response.success) {
          ...
          setAdditionalData({ ...additionalData, employeeData: response?.data?.employee });
          setStatuses({ ...statuses, can_change_status: response?.data?.can_change_status, plan_status: response?.data?.plan_status });
          setPagination({ ...pagination, total: response.data?.employeePlans?.total });
      ...
      <Grid container padding={2.4} spacing={gridSpacing}>
        <Grid item xs={12} sm={12} md={6} lg={3} xl={3}>
          ...
          {!smallDevice && (
            <Box sx={{ paddingTop: 3 }}>
              <Conversations
                url={Backend.api + Backend.getEvaluationApprovalRemark + state.id}
                taskID={state.id}
          ...
          <Grid container>
            <Grid item xs={12}>
              <Grid container spacing={gridSpacing} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Grid item xs={12} sm={12} md={12} lg={9} xl={9} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                  <ApprovalActionButtons
                    onAmend={
                      allowedStatus.includes('amended')
                        ? () => handleOpenModal(managerLevel === 'first' ? 'Sending Back to Employee' : 'Amending', 'amended')
                        : null
                    }
                    onApprove={allowedStatus.includes('approved') ? () => handleOpenModal('Approving', 'approved') : null}
                    level={managerLevel}
                  />
                </Grid>
              </Grid>
