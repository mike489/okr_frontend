...
import ReportForm from './components/CreateDiskReports';
...
const PlanReports = () => {
 ...
 const handleDiskReportAddition = async (values) => {
  ...
  try {
    ...
    console.log('Submitting report with values:', formattedValues); // Debug log
    ...
    if (responseData.success) {
      toast.success(responseData.message || 'Disk Report created successfully');
      handleCreateDiskModalClose();
      handleFetchingReport();
    } else {
      toast.error(responseData.message || 'Failed to create disk report');
    }
  } catch (error) {
  ...
  const handleFetchingReport = async () => {
    setLoading(true);
    const token = await GetToken();
    const Api = Backend.api + Backend.reporting + `?page=${pagination.page + 1}&per_page=${pagination.per_page}`;
    const header = {
      Authorization: `Bearer ${token}`,
      accept: 'application/json',
      'Content-Type': 'application/json',
  ...
  return (
    <PageContainer
      ...
      rightOption={
        hasPermission('create:reporting') && (
          <Box
            display="flex"
            flexDirection="row"
            alignItems="center"
          ...
          >
            <SplitButton
              options={['Desk Report', 'Import From Excel']}
