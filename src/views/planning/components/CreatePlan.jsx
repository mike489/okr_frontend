...
export const CreatePlan = ({ add, unit_id, unit_type, onClose, onSucceed }) => {
  const theme = useTheme();
  const SelectFiscalYear = useSelector((state) => state.customization.selectedFiscalYear);
  const { selectedKpi, selectedPerspective, selectedObjective } = useKPI();
  ...
  const handleNext = () => {
    if (activeIndex === 0) {
      ...
      if (!selectedPerspective.id) {
        setError({ ...error, state: true, message: 'The perspective field is required' });
      } else if (!selectedObjective.id) {
        setError({ ...error, state: true, message: 'The objective field is required' });
