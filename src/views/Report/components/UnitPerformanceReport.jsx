...
const UnitPerformanceReport = ({ id }) => {
  ...
  useEffect(() => {
    const handleFetchingPerformance = async () => {
      if (selectedYear?.id) {
        ...
        const Api = Backend.api + Backend.unitPerformance + id + `?fiscal_year_id=${selectedYear?.id}&filter_by=${filter.frequencies}`;
  ...
  return (
    <Grid item xs={11.9}>
      ...
      {selectedPeriod?.id && <UnitPerKPIPerformance id={id} selectedPeriod={selectedPeriod} filtered_by={filter.frequencies} />}
    </Grid>
  );
};
