...
const ManagerUnitPerformance = () => {
  ...
  useEffect(() => {
    const handleFetchingPerformance = async () => {
      if (selectedYear?.id) {
        ...
        const Api = Backend.api + Backend.myUnitPerformance + `?fiscal_year_id=${selectedYear?.id}`;
  ...
  return (
    <Grid item xs={11.9}>
      <Grid container spacing={gridSpacing}>
        <Grid item xs={12} sm={12} md={6} lg={4} xl={4}>
          <DrogaCard sx={{ mt: 2 }}>
            ...
            <Grid container sx={{ marginTop: 2, borderTop: 0.8, borderColor: theme.palette.divider, padding: 2 }}>
              {isLoading ? (
                ...
                <Grid item xs={12} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  <DrogaDonutChart value={parseFloat(overallPerformance.overall).toFixed(1)} size={68} color={overallPerformance.color} />
      ...
      {selectedPeriod?.id && <ManagerPerKpiPerformance selectedPeriod={selectedPeriod} />}

      <Grid container sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Grid item xs={12} sx={{ my: 2 }}>
          <MonthlyTrends title="Monthly Trends" url={Backend.api + Backend.myUnitMonthlyTrends} itshows="Performance" />
        </Grid>
      </Grid>
    </Grid>
  );
