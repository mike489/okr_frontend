...
const KPIManagement = () => {
  ...
  const [perspectivesSelector] = useState([
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setDeleteKPI(false);
        setData((prevKpis) =>
          prevKpis.filter((kpi) => kpi.id !== selectedKPI?.id),
        );
      } else {
        setDeleting(false);
        toast.error(response.data.data.message);
  ...
  const handleSettingUpVariationFilter = (variations) => {
    variationCategory.length < 2 &&
      variations.forEach((variation) =>
        variationCategory.push({ label: variation, value: variation }),
      );
  };

  const handleSettingUpCalculationFilter = (calculations) => {
    calculationType.length < 2 &&
      calculations.forEach((calculation) =>
        calculationType.push({ label: calculation, value: calculation }),
      );
  };
    ...
    <PageContainer
      ...
      rightOption={
        hasPermission('create:kpi') && (
          <SplitButton
            options={AddKpiOptions}
            handleSelection={(value) => handleKpiAdd(value)}
          />
        )
      }
    >
            ...
            <Grid item xs={12} sm={12}>
                ...
                >
                  <ButtonBase
                    ...
                    sx={{
                      ...
                      backgroundColor:
                        view === 'list' && theme.palette.grey[100],
                    }}
                  >
                    <IconLayoutList
                      stroke={1.6}
                      size="1.4rem"
                      style={{
                        color: view === 'list' && theme.palette.primary[800],
                      }}
                    />
                  </ButtonBase>
                  ...
                  <ButtonBase
                    ...
                    sx={{
                      ...
                      transition: 'all 0.4s ease',
                      backgroundColor:
                        view === 'card' && theme.palette.grey[100],
                    }}
                  >
                    <IconLayoutGrid
                      stroke={1.6}
                      size="1.4rem"
                      style={{
                        color: view === 'card' && theme.palette.primary[800],
                      }}
                    />
                  </ButtonBase>
                </Grid>
              ...
              {loading ? (
                ...
                <ErrorPrompt
                  image={noresult}
                  title="Server Error"
                  message="Unable to retrive the KPI's!"
                />
              ) : data.length === 0 ? (
                <Fallbacks
                  severity="kpis"
                ...
                <ListView
                  data={data}
                  onEdit={(kpi) => handleOpenUpdate(kpi)}
                  onDelete={(kpi) => handleDeleteKPI(kpi)}
                />
              ) : (
                <Grid
                  container
