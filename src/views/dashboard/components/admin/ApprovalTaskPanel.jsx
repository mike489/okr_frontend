...
export const ApprovalTaskPanel = () => {
  ...
  const handleFetchingApprovalTasks = async () => {
    try {
      ...
      const Api = Backend.api + Backend.myTeams + `?fiscal_year_id=${selectedYear?.id}`;
      const header = {
        ...
        <>
          {data.map((task, index) => (
            <TaskListComponent
              ...
              title={task.name}
              unit={task.unit}
              sub_task_count={task?.sub_tasks_count}
              hoverColor={theme.palette.grey[50]}
              onPress={() => navigate('/my-team/member/tasks', { state: { ...task, id: task.id} })}
            />
          ))}
