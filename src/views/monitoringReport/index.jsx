...
const MonitoringReport = () => {
  ...
  const handleGettingMonitoringReportCount = async () => {
    try {
      // Extract the month name from the selectedMonth object
      const monthName = selectedMonth ? format(selectedMonth, 'MMMM') : '';

      // Construct the API URL with fiscal_year_id and month
  ...
  const handleGettingData = async () => {
    try {
      ...
      const employees = `${Backend.api}${Backend.employeeMonitoringList}?fiscal_year_id=${selectedYear?.id}&page=${pagination.page + 1}&per_page=${pagination.per_page}&filter=${filter}&search=${search}`;
      const units = `${Backend.api}${Backend.unitMonitoringList}?fiscal_year_id=${selectedYear?.id}&page=${pagination.page + 1}&per_page=${pagination.per_page}&filter=${filter}&search=${search}`;
      const Api = selection === 'employees' ? employees : units;
              ...
              <Grid item xs={12} sm={6} md={3} lg={3} xl={3} key={index}>
                <Box
                  sx={{
                    ...
                    '&:hover': {
                      backgroundColor: 'rgba(108, 93, 211, 0.9)',
                      boxShadow: '0 4px 15px rgba(108, 93, 211, 0.5)',
                    },
                    cursor: 'pointer',
                    border: 'none',
                    p: 2,
      ...
      <Grid container mt={1} spacing={1}>
        <Grid item xs={12}>
          {loading ? (
            ...
            selection === 'employees' ? (
              <EmployeeMonitoringTable data={data} />
            ) : (
              <ManagersMonitoringTable data={data} />
            )
          ) : (
            <Fallbacks
              severity={selection}
