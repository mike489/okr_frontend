...
const MyObjective = () => {
  ...
  const handleFetchingMyObjective = async () => {
    setLoading(true);
    const token = await GetToken();
    const Api =
      Backend.pmsUrl(Backend.myObjectives) +
  ...
  const handleObjectiveUpdate = async (values) => {
    ...
    const Api = Backend.pmsUrl(
      `${Backend.objectives}/${selectedObjective?.id}`,
    );
          ...
          <Grid container>
            <Grid item xs={12}>
              {loading ? (
                ...
                >
                  <Table aria-label="My Objective table" sx={{ minWidth: 900 }}>
                    <TableHead>
                      <TableRow>
