...
const validationSchema = Yup.object().shape({
  ...
  start_date: Yup.string().optional(), // Made optional since started_date is missing
});
...
const UpdateEmployee = ({ update, isUpdating, onClose, EmployeeData, handleSubmission }) => {
  ...
  // Log EmployeeData to debug its structure
  React.useEffect(() => {
    handleFetchingTypes();
    handleFetchingRoles();
    fetchUnitTypeId();
    setSelectedRoles(EmployeeData?.roles || []);
    setRoleIds(EmployeeData?.roles?.map((role) => role.uuid || role.id) || []); 
    if (!update) {
      formik.resetForm();
      setSelectedRoles([]);
      setRoleIds([]);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      type: EmployeeData?.unit_type_id || '', 
      unit: EmployeeData?.unit_id || '',
      start_date: EmployeeData?.started_date || '',
    ...
    onSubmit: async (values) => {
      console.log('Submitted values:', values, 'Role IDs:', roleIds);
      try {
        await handleSubmission(values, roleIds);
      } catch (error) {
  ...
  const handleRoleSelection = (event, value) => {
    ...
    setRoleIds(value.map((role) => role.uuid));
  };
  ...
  // Fetch unit_type_id for the employee's unit_id
  const fetchUnitTypeId = async () => {
    if (!EmployeeData?.unit_id) return;
    try {
      const token = await GetToken();
      const response = await fetch(`${Backend.api}units/${EmployeeData.unit_id}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
          accept: 'application/json',
      ...
      const data = await response.json();
      if (data.success && data.data.unit_type_id) {
        formik.setFieldValue('type', data.data.unit_type_id);
      }
    } catch (error) {
      toast.error('Error fetching unit type');
    }
  };
  ...
  React.useEffect(() => {
    handleFetchingTypes();
    handleFetchingRoles();
    fetchUnitTypeId(); // Fetch unit_type_id when component mounts
    setSelectedRoles(EmployeeData?.roles || []);
    setRoleIds(EmployeeData?.roles?.map((role) => role.uuid) || []);
    if (!update) {
      formik.resetForm();
      setSelectedRoles([]);
      setRoleIds([]);
  ...
  // Show error if EmployeeData is incomplete
  if (!EmployeeData) {
    return (
      <Dialog open={update} onClose={onClose}>
        <DialogTitle>Error</DialogTitle>
        <DialogContent>
          <Typography color="error">Employee data not available</Typography>
        </DialogContent>
        <DialogActions>
