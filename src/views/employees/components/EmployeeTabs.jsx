      ...
      <Box>
        <AntTabs value={value} onChange={handleChange} aria-label="Employee tabs" theme={theme}>
          {EmployeeTabs.map((tab, index) => (
            <AntTab key={index} label={tab.label} icon={tab.icon} iconPosition="start" {...a11yProps(index)} color="text.primary" />
          ))}
        </AntTabs>
      </Box>
      ...
      <TabPanel value={value} index={2} dir={theme.direction}>
        ...
        <Grid container sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Grid item xs={12} sx={{ my: 2 }}>
            <MonthlyTrends title="Monthly Trends" url={Backend.api + Backend.employeeMonthlyTrends + id} itshows="Performance" />
          </Grid>
        </Grid>
      </TabPanel>
