            ...
            performance.map((kpi, index) => (
              <DrogaCard key={index} sx={{ marginY: 1.4, py: 1.4 }}>
                <Grid container spacing={gridSpacing} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <Grid item xs={12} sm={6} md={2} lg={2} xl={2} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                    <DrogaDonutChart value={parseFloat(kpi.kpi_performance).toFixed(1)} size={28} color={kpi?.color} />
                  </Grid>
                </Grid>
              </DrogaCard>
            ))
